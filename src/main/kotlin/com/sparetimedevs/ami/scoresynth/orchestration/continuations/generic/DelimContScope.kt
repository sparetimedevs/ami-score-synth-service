 ...
 */
internal open class DelimContScope<R>(
    ...
    public data class SingleShotCont<A, R>(
        ...
        public override suspend fun invoke(a: A): R =
            suspendCoroutineUninterceptedOrReturn { resumeShift ->
     ...
     */
    public suspend fun <A, B> shiftCPS(
        f: suspend (DelimitedContinuation<A, B>) -> R,
        c: suspend DelimitedScope<B>.(A) -> B,
    ): Nothing =
     ...
     */
    fun <A> reset(f: suspend DelimitedScope<A>.() -> A): A = DelimContScope(f).invoke()
        ...
        f
            .startCoroutineUninterceptedOrReturn(
                ...
                if (it == COROUTINE_SUSPENDED) {
                    ...
                    return@invoke it as R // we can return directly if we never suspended/called shift
                }
            }
